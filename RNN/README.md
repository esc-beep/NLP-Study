...
RNN의 학습도 기본적으로 FFN과 비슷하다. 훈련 세트(training set)가 필요하고, 모델이 얼마나 틀렸는지 측정하는 손실 함수(loss function)가 필요하며, 틀린 값을 바탕으로 가중치를 수정하는 역전파를 사용한다.<br>
RNN의 가중치는 다음과 같다.
...
역전파(Backpropagation)는 인공신경망이 학습할 때, 가중치(weight) 를 업데이트하기 위해 사용하는 핵심 알고리즘이다.<br>
신경망의 출력값과 실제 정답(레이블) 의 차이, 즉 손실(Loss) 을 최소화하기 위해 오차를 출력층에서 입력층 방향으로 전파하면서 각 가중치를 조정한다.<br>
이 과정에서 연쇄 법칙(Chain Rule) 을 이용하여, 각 가중치가 최종 손실에 얼마나 기여했는지(민감도) 를 계산한다.<br>
이 값이 바로 기울기(Gradient) 이며, 기울기는 손실을 줄이기 위한 가중치의 변화 방향과 크기를 알려준다.<br>
최종적으로, 역전파는 이 기울기를 이용해 손실을 줄이는 방향으로 가중치를 업데이트한다.<br>
경사하강법(Gradient Descent) 같은 최적화 알고리즘과 함께 작동한다.<br>
